...
- **Right-pane seats are not implemented**: `@deepseek-ai/dsh-client-ui-sidebar-right` and its `dsh-client-ui-dockkit` dependency are not published to npm, so an out-of-tree client plugin cannot declare `sidebar.right.pane.tab`. Citations open the library page through the `med-knowledge` paper-focus contract instead (the reader is a section of that page, not a view of its own); see `docs/decisions/2026-09-08-citation-focus.md` and `docs/decisions/2026-09-17-shell-prototype-alignment.md`.
- **Only five views are registered**: every `conversation.view` entry becomes a host tab, so the package registers exactly the five prototype entries (home, research, library, statistics, skills). The paper reader (S03), the evidence list (S04), and the draft editor (S08) are page sections addressed through an opaque focus (`encodePaperFocus` / `encodeClaimFocus`); see `docs/decisions/2026-09-17-shell-prototype-alignment.md`.
