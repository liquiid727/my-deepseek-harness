...
- **Emphasis uses the semantic accent.** A hovered divider, the drop caret, and the drop-zone hint use `--dsw-alias-accent-primary`, as the trajectory views do. A floating panel's border is the same `--dsw-alias-border-l2` hairline whether it is active or not: the active panel is already on top and casts the shadow; a darker frame around it reads as a defect.
