...
A Conversation View can place the resident composer in its own layout with the `mountComposer` owner callback. It supplies the id of a mounted empty element, a localized placeholder, and an `onMessageAccepted` callback, and releases the registration on unmount. The shell moves one portal container, retaining the editor and its draft; pending interactions return it to the dock. Ordinary sends invoke the callback only after admission while the originating Session, View, and registration remain current. Failed sends restore the draft through the normal input flow; slash commands do not invoke this callback. See the [placement decision](../../../.agents/notes/implemented/architecture/2026-09-13-view-composer-outlet.md).
