...
Session 首次绑定或缓存的 Session 成为 current 时，shell 会在渲染前读取持久化 View 偏好，激活已注册的偏好 View 或 Chat fallback，并在后续 tab 或 focus 选择写入 store 前先激活对应 target。blank Session 会渲染 `conversation.hero.actions`；启动控件可以在首条模型消息前选择 View，未选中的 target 不会激活。
