...
requirement: R001
spec_package: S07
test_spec_id: TEST-R001-S07
source_prd: ../../prd.md
source_spec: ./spec.md
source_spec_id: SPEC-R001-S07
...
# Test Design — S07 Skills Center and Builder
...
In scope: REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018；AC-R001-011, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。实现耦合单测覆盖数据/算法，独立 Service/Remote、profile、浏览器、Runner 与医学评审分别覆盖实际消费行为。代码实现、生产上线、真实患者数据和 R001 Non-Goals 不属于此文档操作范围。

## 2. Coverage Matrix

| Requirement | Spec behavior | Planned Test | Level / evidence / gate |
|---|---|---|---|
| REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018 | SPEC-R001-S07-001 | TEST-R001-S07-006 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018 | SPEC-R001-S07-002 | TEST-R001-S07-007 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018 | SPEC-R001-S07-003 | TEST-R001-S07-008 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018 | SPEC-R001-S07-004 | TEST-R001-S07-009 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009 | 全部 UI 行为 | TEST-R001-S07-004 | 真实浏览器截图/操作 trace/console；blocking |
| REQ-R001-018 | 全部注册与模型可见行为 | TEST-R001-S07-003, TEST-R001-S07-005 | profile/Session replay/disposal；blocking |

BR-R001-001..005、INV-R001-001..007、EDGE-R001-001..010 按责任适用：所有包承担范围/来源/回放/并发/窄屏，S01 承担无 Session/导入，S02/S04 承担 Connector partial，S03/S04/S06/S08 承担 anchor/Claim/Draft，S05 承担 Runner/行隐私，S07 承担 Skill 扩权。无该领域 API 的包不重复执行其负例，但 TEST-R001-S07-005 必须引用对应 owner 的集成结果。每项 AC-R001-011, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016 都需当前绑定结果。

## 3. Test Environment and Data

- 数据集计划名称 R001-S07-contract-v1，包含下方每个命名输入与负例；实现测试资产时记录精确文件/hash。这些名称是待创建的验证数据，不声称现有 fixture 已覆盖。
...
### TEST-R001-S07-001 Primary outcome

Given 下方正例输入与独立 Project，When 通过 Skill catalog/details 完成该业务并重载，Then 用户可见结果与各 owned record 的 ID/version/来源一致；不能只检查模型声称成功。覆盖本包全部 REQ/AC 的主流程，由 006 起的精确行为用例提供分项断言。

### TEST-R001-S07-002 Failure honesty

Given 下方每项负例（13 内置定义、多个版本、草稿、disabled/active/revoked 安装；全部 Builder 字段、JSON Schema 2020-12、无效 tool/model/$ref/example/knowledge；公开 sample PDF、schema 输入、允许/禁止工具、延迟/失败 Provider；每个内置 Skill 的目录正例/缺失输入/不足/越权案例；旧 active version），When 调用对应公开操作，Then 返回 Spec 声明错误/partial，成功记录保留、非法写为零。每个负例单列输入、实际 code、前后状态，不把一次通用失败代表所有失败分支。

### TEST-R001-S07-003 Invariant, retry, and isolation
...
### TEST-R001-S07-004 Asset-aligned browser UI
...
### TEST-R001-S07-005 Supported-profile release scenario
...
### TEST-R001-S07-006 Catalog/inventory

Covers: SPEC-R001-S07-001；REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018；AC-R001-011, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 13 内置定义、多个版本、草稿、disabled/active/revoked 安装。

When: 通过 Skill catalog/details，分类搜索、我的/已安装切换、详情、检查有效权限。

Then: 13 项存在且 source/version/schema/permissions 可见，update-available 派生正确，列表不是静态 mock。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S07-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S07-007 Builder validation

Covers: SPEC-R001-S07-002；REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018；AC-R001-011, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 全部 Builder 字段、JSON Schema 2020-12、无效 tool/model/$ref/example/knowledge。

When: 通过 Skill drafts.save/validate，逐字段保存重载、编辑已验证 revision、提交非法定义。

Then: 字段原样保存与精确错误 path；未知权限不能通过；编辑清除旧验证/test freshness。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S07-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S07-008 Controlled test

Covers: SPEC-R001-S07-003；REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018；AC-R001-011, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 公开 sample PDF、schema 输入、允许/禁止工具、延迟/失败 Provider。

When: 通过 Skill test/cancel、临时 registry，运行并取消/失败/重试、切换 Preview、读取生产对象。

Then: 真实 progress/结果，output schema 校验；production 零写入；权限交集，网络与统计审批仍阻塞；资源清理。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S07-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S07-009 Publish/install and 13 Skills

Covers: SPEC-R001-S07-004；REQ-R001-009, REQ-R001-010, REQ-R001-016, REQ-R001-018；AC-R001-011, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 每个内置 Skill 的目录正例/缺失输入/不足/越权案例；旧 active version。

When: 通过 Skill publish/install/enable/upgrade/uninstall/revoke，发布同版本异内容、安装/显式启用、扩权拒绝/升级失败/撤销在途；逐项执行业务。

Then: VERSION_EXISTS；默认 disabled；拒绝保留旧版；撤销阻止新调用并取消在途；13 份独立语义断言与医学评审，不能平均掩盖失败。
