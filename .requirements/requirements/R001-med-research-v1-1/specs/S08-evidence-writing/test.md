...
requirement: R001
spec_package: S08
test_spec_id: TEST-R001-S08
source_prd: ../../prd.md
source_spec: ./spec.md
source_spec_id: SPEC-R001-S08
...
# Test Design — S08 Evidence-based Writing and Export
...
In scope: REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018；AC-R001-008, AC-R001-012, AC-R001-014, AC-R001-015, AC-R001-016。实现耦合单测覆盖数据/算法，独立 Service/Remote、profile、浏览器、Runner 与医学评审分别覆盖实际消费行为。代码实现、生产上线、真实患者数据和 R001 Non-Goals 不属于此文档操作范围。

## 2. Coverage Matrix

| Requirement | Spec behavior | Planned Test | Level / evidence / gate |
|---|---|---|---|
| REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018 | SPEC-R001-S08-001 | TEST-R001-S08-006 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018 | SPEC-R001-S08-002 | TEST-R001-S08-007 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018 | SPEC-R001-S08-003 | TEST-R001-S08-008 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009 | 全部 UI 行为 | TEST-R001-S08-004 | 真实浏览器截图/操作 trace/console；blocking |
| REQ-R001-018 | 全部注册与模型可见行为 | TEST-R001-S08-003, TEST-R001-S08-005 | profile/Session replay/disposal；blocking |

BR-R001-001..005、INV-R001-001..007、EDGE-R001-001..010 按责任适用：所有包承担范围/来源/回放/并发/窄屏，S01 承担无 Session/导入，S02/S04 承担 Connector partial，S03/S04/S06/S08 承担 anchor/Claim/Draft，S05 承担 Runner/行隐私，S07 承担 Skill 扩权。无该领域 API 的包不重复执行其负例，但 TEST-R001-S08-005 必须引用对应 owner 的集成结果。每项 AC-R001-008, AC-R001-012, AC-R001-014, AC-R001-015, AC-R001-016 都需当前绑定结果。

## 3. Test Environment and Data

- 数据集计划名称 R001-S08-contract-v1，包含下方每个命名输入与负例；实现测试资产时记录精确文件/hash。这些名称是待创建的验证数据，不声称现有 fixture 已覆盖。
...
### TEST-R001-S08-001 Primary outcome

Given 下方正例输入与独立 Project，When 通过 writing.generate/validate、S06 drafts 完成该业务并重载，Then 用户可见结果与各 owned record 的 ID/version/来源一致；不能只检查模型声称成功。覆盖本包全部 REQ/AC 的主流程，由 006 起的精确行为用例提供分项断言。

### TEST-R001-S08-002 Failure honesty

Given 下方每项负例（SUPPORT/AGAINST/SECONDARY/withdrawn，支持与不足段落；zh/en 段落、表格、OR=1.42、95% CI 与引用；无来源自由文本；真实 metadata 含 Unicode/花括号/换行、缺作者/年份/标题，Draft stale reference），When 调用对应公开操作，Then 返回 Spec 声明错误/partial，成功记录保留、非法写为零。每个负例单列输入、实际 code、前后状态，不把一次通用失败代表所有失败分支。

### TEST-R001-S08-003 Invariant, retry, and isolation
...
### TEST-R001-S08-004 Asset-aligned browser UI
...
### TEST-R001-S08-005 Supported-profile release scenario
...
### TEST-R001-S08-006 Draft generation/edit validation

Covers: SPEC-R001-S08-001；REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018；AC-R001-008, AC-R001-012, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: SUPPORT/AGAINST/SECONDARY/withdrawn，支持与不足段落。

When: 通过 writing.generate/validate、S06 drafts，生成综述、改变事实数字、解除引用、编辑标题、撤销来源。

Then: 事实绑定当前 Evidence；改变事实/解除引用即 DRAFT；纯格式不丢来源；冲突双方仍可验证，缺失不生成肯定结论。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S08-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S08-007 Academic translation

Covers: SPEC-R001-S08-002；REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018；AC-R001-008, AC-R001-012, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: zh/en 段落、表格、OR=1.42、95% CI 与引用；无来源自由文本。

When: 通过 writing.translate，双向翻译、注入数字/引用对齐错误、修正重验。

Then: 结构/数字/引用 identity 保持；TRANSLATION_MISMATCH 不覆盖旧版；自由文本不冒充 verified Draft。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S08-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S08-008 Export

Covers: SPEC-R001-S08-003；REQ-R001-006, REQ-R001-009, REQ-R001-012, REQ-R001-017, REQ-R001-018；AC-R001-008, AC-R001-012, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 真实 metadata 含 Unicode/花括号/换行、缺作者/年份/标题，Draft stale reference。

When: 通过 writing.export、鉴权 Artifact 下载，三格式解析、complete/preview、跨 Project 下载、导出提交前撤销 Evidence。

Then: 字段可解析且不补造；缺 title METADATA_INCOMPLETE；stale complete 拒绝；preview 标 incomplete；跨 scope 拒绝且无半成文件。
