...
requirement: R001
spec_package: S06
test_spec_id: TEST-R001-S06
source_prd: ../../prd.md
source_spec: ./spec.md
source_spec_id: SPEC-R001-S06
...
# Test Design — S06 Knowledge and Library
...
In scope: REQ-R001-001, REQ-R001-009, REQ-R001-012, REQ-R001-015, REQ-R001-017, REQ-R001-018；AC-R001-008, AC-R001-014, AC-R001-015。实现耦合单测覆盖数据/算法，独立 Service/Remote、profile、浏览器、Runner 与医学评审分别覆盖实际消费行为。代码实现、生产上线、真实患者数据和 R001 Non-Goals 不属于此文档操作范围。

## 2. Coverage Matrix

| Requirement | Spec behavior | Planned Test | Level / evidence / gate |
|---|---|---|---|
| REQ-R001-001, REQ-R001-009, REQ-R001-012, REQ-R001-015, REQ-R001-017, REQ-R001-018 | SPEC-R001-S06-001 | TEST-R001-S06-006 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-001, REQ-R001-009, REQ-R001-012, REQ-R001-015, REQ-R001-017, REQ-R001-018 | SPEC-R001-S06-002 | TEST-R001-S06-007 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-001, REQ-R001-009, REQ-R001-012, REQ-R001-015, REQ-R001-017, REQ-R001-018 | SPEC-R001-S06-003 | TEST-R001-S06-008 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-001, REQ-R001-009, REQ-R001-012, REQ-R001-015, REQ-R001-017, REQ-R001-018 | SPEC-R001-S06-004 | TEST-R001-S06-009 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009 | 全部 UI 行为 | TEST-R001-S06-004 | 真实浏览器截图/操作 trace/console；blocking |
| REQ-R001-018 | 全部注册与模型可见行为 | TEST-R001-S06-003, TEST-R001-S06-005 | profile/Session replay/disposal；blocking |

BR-R001-001..005、INV-R001-001..007、EDGE-R001-001..010 按责任适用：所有包承担范围/来源/回放/并发/窄屏，S01 承担无 Session/导入，S02/S04 承担 Connector partial，S03/S04/S06/S08 承担 anchor/Claim/Draft，S05 承担 Runner/行隐私，S07 承担 Skill 扩权。无该领域 API 的包不重复执行其负例，但 TEST-R001-S06-005 必须引用对应 owner 的集成结果。每项 AC-R001-008, AC-R001-014, AC-R001-015 都需当前绑定结果。

## 3. Test Environment and Data

- 数据集计划名称 R001-S06-contract-v1，包含下方每个命名输入与负例；实现测试资产时记录精确文件/hash。这些名称是待创建的验证数据，不声称现有 fixture 已覆盖。
...
### TEST-R001-S06-001 Primary outcome

Given 下方正例输入与独立 Project，When 通过 medKnowledge.list 与 medLiterature.save/unsave 完成该业务并重载，Then 用户可见结果与各 owned record 的 ID/version/来源一致；不能只检查模型声称成功。覆盖本包全部 REQ/AC 的主流程，由 006 起的精确行为用例提供分项断言。

### TEST-R001-S06-002 Failure honesty

Given 下方每项负例（A/B 共享 Paper，uploaded Paper，仅 A 可用 Evidence；孤立/被引用 source；三类 Note，各有 body/anchor；重复大小写 Tag；title/author/PMID/Note/tag 各有唯一关键词，A/B scope；A 原文/Verified Evidence/无 source Note，B 高相关论文），When 调用对应公开操作，Then 返回 Spec 声明错误/partial，成功记录保留、非法写为零。每个负例单列输入、实际 code、前后状态，不把一次通用失败代表所有失败分支。

### TEST-R001-S06-003 Invariant, retry, and isolation
...
### TEST-R001-S06-004 Asset-aligned browser UI
...
### TEST-R001-S06-005 Supported-profile release scenario
...
### TEST-R001-S06-006 Library memberships

Covers: SPEC-R001-S06-001；REQ-R001-009, REQ-R001-012, REQ-R001-018；AC-R001-008, AC-R001-014, AC-R001-015。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: A/B 共享 Paper，uploaded Paper，仅 A 可用 Evidence；孤立/被引用 source。

When: 通过 medKnowledge.list 与 medLiterature.save/unsave，Project/My/Uploaded 切换、删除 membership、尝试清理被引用 source。

Then: scope 明确；删除 A 不删 B；SOURCE_IN_USE 拒绝；无悬空 Note/Evidence/Draft。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S06-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S06-007 Notes and Tags

Covers: SPEC-R001-S06-002；REQ-R001-009, REQ-R001-012, REQ-R001-018；AC-R001-008, AC-R001-014, AC-R001-015。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 三类 Note，各有 body/anchor；重复大小写 Tag。

When: 通过 medKnowledge note adapter、Tags CRUD、S03 notes，编辑 Note、创建重名 Tag、rename/delete、重开 Reader。

Then: S03 同一 Note ID/version；TAG_EXISTS；rename 更新关联，delete 不删对象；anchor 不变。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S06-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S06-008 Search

Covers: SPEC-R001-S06-003；REQ-R001-009, REQ-R001-012, REQ-R001-018；AC-R001-008, AC-R001-014, AC-R001-015。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: title/author/PMID/Note/tag 各有唯一关键词，A/B scope。

When: 通过 medKnowledge.search，逐字段搜索、空 query、cursor 翻页、选 My Library。

Then: 命中字段与 scope 正确；稳定分页；空 query 有限列表；默认无 B 数据。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S06-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S06-009 RAG and Draft inventory

Covers: SPEC-R001-S06-004；REQ-R001-009, REQ-R001-012, REQ-R001-018；AC-R001-008, AC-R001-014, AC-R001-015。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: A 原文/Verified Evidence/无 source Note，B 高相关论文。

When: 通过 medKnowledge.rag/drafts、S04 gate、S08 validation action，RAG 提问、移除 membership/重建索引、空白 Draft、撤销 Evidence。

Then: 无 B 或模型常识补齐；Note 非唯一事实支持；INDEX_REBUILDING/insufficient 明确；Draft 失效降级并保留历史。
