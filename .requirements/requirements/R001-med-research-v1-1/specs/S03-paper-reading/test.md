...
In scope: REQ-R001-004, REQ-R001-009, REQ-R001-012, REQ-R001-014, REQ-R001-015, REQ-R001-018；AC-R001-004, AC-R001-005, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。实现耦合单测覆盖数据/算法，独立 Service/Remote、profile、浏览器、Runner 与医学评审分别覆盖实际消费行为。代码实现、生产上线、真实患者数据和 R001 Non-Goals 不属于此文档操作范围。

## 2. Coverage Matrix

| Requirement | Spec behavior | Planned Test | Level / evidence / gate |
|---|---|---|---|
| REQ-R001-004, REQ-R001-009, REQ-R001-012, REQ-R001-014, REQ-R001-015, REQ-R001-018 | SPEC-R001-S03-001 | TEST-R001-S03-006 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-004, REQ-R001-009, REQ-R001-012, REQ-R001-014, REQ-R001-015, REQ-R001-018 | SPEC-R001-S03-002 | TEST-R001-S03-007 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-004, REQ-R001-009, REQ-R001-012, REQ-R001-014, REQ-R001-015, REQ-R001-018 | SPEC-R001-S03-003 | TEST-R001-S03-008 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-004, REQ-R001-009, REQ-R001-012, REQ-R001-014, REQ-R001-015, REQ-R001-018 | SPEC-R001-S03-004 | TEST-R001-S03-009 | 独立 Service/Remote + 持久化；逐 feature 断言、响应/状态记录；blocking |
| REQ-R001-009 | 全部 UI 行为 | TEST-R001-S03-004 | 真实浏览器截图/操作 trace/console；blocking |
| REQ-R001-018 | 全部注册与模型可见行为 | TEST-R001-S03-003, TEST-R001-S03-005 | profile/Session replay/disposal；blocking |

BR-R001-001..005、INV-R001-001..007、EDGE-R001-001..010 按责任适用：所有包承担范围/来源/回放/并发/窄屏，S01 承担无 Session/导入，S02/S04 承担 Connector partial，S03/S04/S06/S08 承担 anchor/Claim/Draft，S05 承担 Runner/行隐私，S07 承担 Skill 扩权。无该领域 API 的包不重复执行其负例，但 TEST-R001-S03-005 必须引用对应 owner 的集成结果。每项 AC-R001-004, AC-R001-005, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016 都需当前绑定结果。

## 3. Test Environment and Data

- 数据集计划名称 R001-S03-contract-v1，包含下方每个命名输入与负例；实现测试资产时记录精确文件/hash。这些名称是待创建的验证数据，不声称现有 fixture 已覆盖。
...
### TEST-R001-S03-001 Primary outcome

Given 下方正例输入与独立 Project，When 通过 medPapers.resolve/getDocument/focus 完成该业务并重载，Then 用户可见结果与各 owned record 的 ID/version/来源一致；不能只检查模型声称成功。覆盖本包全部 REQ/AC 的主流程，由 006 起的精确行为用例提供分项断言。

### TEST-R001-S03-002 Failure honesty

Given 下方每项负例（公开授权 PDF、PMC XML、Abstract 与无文本扫描 PDF；Unicode/跨段选区；同一 Document 有/无样本量字段；selection 与 document 两种范围；NFC/CRLF、多段落与键盘选区；Project/Paper/Selection Notes；有效 DOI/PMID 引用、无 ID reference、失效 source version），When 调用对应公开操作，Then 返回 Spec 声明错误/partial，成功记录保留、非法写为零。每个负例单列输入、实际 code、前后状态，不把一次通用失败代表所有失败分支。

### TEST-R001-S03-003 Invariant, retry, and isolation
...
### TEST-R001-S03-004 Asset-aligned browser UI
...
### TEST-R001-S03-005 Supported-profile release scenario
...
### TEST-R001-S03-006 Sources and anchors

Covers: SPEC-R001-S03-001；REQ-R001-004, REQ-R001-009, REQ-R001-014, REQ-R001-018；AC-R001-004, AC-R001-005, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 公开授权 PDF、PMC XML、Abstract 与无文本扫描 PDF；Unicode/跨段选区。

When: 通过 medPapers.resolve/getDocument/focus，逐源解析、选择版本、解析损坏输入与含外部实体 XML。

Then: 仅真实内容；顺序章节/page/normalized map 正确；外部实体零访问；扫描 PDF 无文字动作且可查看原文件。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S03-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S03-007 Reading/summary/QA

Covers: SPEC-R001-S03-002；REQ-R001-004, REQ-R001-009, REQ-R001-014, REQ-R001-018；AC-R001-004, AC-R001-005, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 同一 Document 有/无样本量字段；selection 与 document 两种范围。

When: 通过 medPapers translation/summary/ask，三模式、whole/section、oneSentence/threeMinute/structured；整篇问答与四类医学抽取。

Then: 原文 hash 不变；逐项摘要有 anchor或未报告；范围外段落不进请求；失败保留原文；13 Skill 目录中 S03 输出有独立断言。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S03-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S03-008 Selection/Notes/Highlights

Covers: SPEC-R001-S03-003；REQ-R001-004, REQ-R001-009, REQ-R001-014, REQ-R001-018；AC-R001-004, AC-R001-005, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: NFC/CRLF、多段落与键盘选区；Project/Paper/Selection Notes。

When: 通过 Reader action registry、notes/annotations CRUD，九个动作逐项执行、取消、保存重开和撤销；从 S06 编辑同 Note。

Then: 同一 anchor/ID，取消无写入，Note body 修改不变原文；S04 Evidence 撤销令引用失效；不能创建第二份 Note。

Required evidence: 分项输入/输出断言、服务响应、操作与审计 ID、前后持久状态；UI 行为同时引用 TEST-R001-S03-004，医学判断引用冻结评审结果。每个 coverage feature 单列 PASS/FAIL/BLOCKED/NOT_RUN；无执行结果不得填 PASS。

### TEST-R001-S03-009 Reference and focus

Covers: SPEC-R001-S03-004；REQ-R001-004, REQ-R001-009, REQ-R001-014, REQ-R001-018；AC-R001-004, AC-R001-005, AC-R001-013, AC-R001-014, AC-R001-015, AC-R001-016。关联 [coverage](../../coverage.md) 中指向此 TEST 的每一行。

Category / level: 独立 Service/Remote 集成与负例；Owner: med-research-testing；Gate: blocking。

Given: 有效 DOI/PMID 引用、无 ID reference、失效 source version。

When: 通过 Reader focus/reference actions，打开 Reference Explorer/Related/source，关闭 drawer，加载 stale focus。

Then: 有效来源精确高亮，未解析不造 ID；联网按 S02 确认；stale 明确失败；焦点回原触发器。
