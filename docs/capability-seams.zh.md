...
flowchart LR
  ...
  pkg_workbench_bridge["workbench-bridge"]
  svc_piBridge["ctx.piBridge<br/>Embedded Pi runtime bridge"]
  ...
  pkg_workbench_bridge --> svc_piBridge
...
| `ctx.piBridge` | `core` | [`workbench-bridge`](../packages/workbench/dsh-bridge) | - | `apiproxy` | - | 将每个 DSH 会话映射到 Pi AgentSession，把标准化 Workbench 事件流投影到 DSH 会话日志，并在 Host 组合时 opt-in。 |
