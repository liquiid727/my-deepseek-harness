...
| `ctx.piBridge` | `core` | [`workbench-bridge`](../packages/workbench/dsh-bridge) | - | `apiproxy` | - | Maps each DSH session to a Pi AgentSession, projects the normalized Workbench event stream into the DSH session log, and is opt-in at Host composition. |
